...
class SphericalVoronoiApp:
    ...
    def setup_gui(self):
        ...
        info_frame.grid(row=1, column=0, columnspan=5, pady=(10, 0), sticky=(tk.W, tk.E))
        
        self.empty_circle_label = ttk.Label(info_frame, text="Largest Empty Circle Radius: Not calculated", 
                                          font=('Arial', 10, 'bold'))
    ...
    def generate_random_points(self):
        ...
        self.largest_empty_circle = None
        self.empty_circle_radius = 0.0
        self.empty_circle_label.config(text="Largest Empty Circle Radius: Not calculated")
    ...
    def find_largest_empty_circle(self, points):
        ...
        for _ in range(20):  # Multiple random starts
            ...
            try:
                result = minimize(objective, [theta_start, phi_start], 
                                method='L-BFGS-B',
    ...
    def display_points_only(self):
        ...
        self.ax.scatter(points[:, 0], points[:, 1], points[:, 2], 
                      c='red', s=120, alpha=0.9, label='Seed Points', edgecolors='black')
    ...
    def draw_circle_on_sphere(self, center, radius, ax):
        ...
        try:
            ...
            ax.plot(circle_points[:, 0], circle_points[:, 1], circle_points[:, 2], 
                   'g-', linewidth=3, label='Largest Empty Circle')
            
            # Plot the center
            ax.scatter([center[0]], [center[1]], [center[2]], 
                      c='green', s=100, marker='*', label='Circle Center')
    ...
    def plot_voronoi(self):
        ...
        try:
            ...
            empty_center, radius_rad, radius_deg = self.find_largest_empty_circle(self.seed_points)
            self.largest_empty_circle = empty_center
            self.empty_circle_radius = radius_rad
            
            print(f"Empty circle found: center={empty_center is not None}, radius={radius_rad}")  # Debug
            ...
            if empty_center is not None and radius_rad > 0:
                ...
                self.empty_circle_label.config(text="Largest Empty Circle Radius: Calculation failed")
            ...
            if hasattr(sv, 'vertices') and len(sv.vertices) > 0:
                ...
                self.ax.scatter(sv.vertices[:, 0], sv.vertices[:, 1], sv.vertices[:, 2], 
                              c='blue', s=60, alpha=0.7, label='Voronoi Vertices')
