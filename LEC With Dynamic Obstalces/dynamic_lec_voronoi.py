...
class DynamicLECVoronoi:
    ...
    def setup_gui(self):
        ...
        title_label = tk.Label(header_frame, text="Dynamic LEC with Voronoi Diagrams", 
                              font=('Arial', 16, 'bold'), bg='white', fg='#333')
        title_label.pack(side=tk.LEFT, padx=10, pady=10)
        
        info_button = tk.Button(header_frame, text="Info", command=self.toggle_info,
...
• Convex Hull (Green): Convex hull of all sites"""
        ...
        tk.Label(self.info_frame, text=info_text, bg='#e3f2fd', fg='#1976d2', 
    ...
    def setup_control_panel(self, parent):
        ...
        # Title
        title_label = tk.Label(control_frame, text="Controls", font=('Arial', 14, 'bold'), 
                              bg='white', fg='#333')
        title_label.pack(pady=10)
        
        # Mode Selection
        mode_frame = tk.LabelFrame(control_frame, text="Mode", bg='white', font=('Arial', 10, 'bold'))
        mode_frame.pack(fill=tk.X, padx=10, pady=5)
        
        button_frame = tk.Frame(mode_frame, bg='white')
        button_frame.pack(fill=tk.X, padx=5, pady=5)
        
        self.sites_button = tk.Button(button_frame, text="Add Sites", 
                                     command=lambda: self.set_mode('sites'),
                                     bg='#e74c3c', fg='white', font=('Arial', 10))
        self.sites_button.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        
        self.obstacles_button = tk.Button(button_frame, text="Add Obstacles", 
                                         command=lambda: self.set_mode('obstacles'),
                                         bg='#95a5a6', fg='white', font=('Arial', 10))
        self.obstacles_button.pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        # Animation Controls
        anim_frame = tk.LabelFrame(control_frame, text="Animation", bg='white', font=('Arial', 10, 'bold'))
        ...
        self.play_button = tk.Button(button_frame2, text="Play", 
                                    command=self.toggle_animation,
                                    bg='#27ae60', fg='white', font=('Arial', 10))
        self.play_button.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        
        reset_button = tk.Button(button_frame2, text="Reset", 
        ...
        tk.Label(speed_frame, text="Speed:", bg='white', font=('Arial', 10)).pack(side=tk.LEFT)
        self.speed_var = tk.DoubleVar(value=1.0)
        self.speed_scale = tk.Scale(speed_frame, from_=0.1, to=3.0, resolution=0.1, 
                                   orient=tk.HORIZONTAL, variable=self.speed_var,
                                   bg='white', font=('Arial', 9))
        self.speed_scale.pack(side=tk.RIGHT, fill=tk.X, expand=True)
        
        # Display Options
        display_frame = tk.LabelFrame(control_frame, text="Display", bg='white', font=('Arial', 10, 'bold'))
        display_frame.pack(fill=tk.X, padx=10, pady=5)
        
        self.voronoi_var = tk.BooleanVar(value=True)
        tk.Checkbutton(display_frame, text="Voronoi Diagram", variable=self.voronoi_var,
                      command=self.update_display, bg='white', font=('Arial', 10)).pack(anchor=tk.W, padx=5, pady=2)
        
        self.hull_var = tk.BooleanVar(value=True)
        tk.Checkbutton(display_frame, text="Convex Hull", variable=self.hull_var,
                      command=self.update_display, bg='white', font=('Arial', 10)).pack(anchor=tk.W, padx=5, pady=2)
        
        self.lec_var = tk.BooleanVar(value=True)
        tk.Checkbutton(display_frame, text="Largest Empty Circle", variable=self.lec_var,
                      command=self.update_display, bg='white', font=('Arial', 10)).pack(anchor=tk.W, padx=5, pady=2)
        
        # Statistics
        stats_frame = tk.LabelFrame(control_frame, text="Statistics", bg='white', font=('Arial', 10, 'bold'))
        stats_frame.pack(fill=tk.X, padx=10, pady=5)
        
        self.stats_label = tk.Label(stats_frame, text="Sites: 0\nObstacles: 0\nTime: 0.00s", 
                                   bg='white', font=('Arial', 10), justify=tk.LEFT)
        self.stats_label.pack(padx=5, pady=5)
        
        # Obstacles List
        self.obstacles_list_frame = tk.LabelFrame(control_frame, text="Obstacles", bg='white', font=('Arial', 10, 'bold'))
        self.obstacles_list_frame.pack(fill=tk.X, padx=10, pady=5)
        
        self.obstacles_listbox = tk.Listbox(self.obstacles_list_frame, height=4, font=('Arial', 9))
    ...
    def toggle_info(self):
        ...
        if self.show_info:
            self.info_frame.pack(fill=tk.X, pady=(0, 10), before=self.root.winfo_children()[0].winfo_children()[2])
        else:
    ...
    def update_display(self):
        ...
        print(f"Display updated: Voronoi={self.show_voronoi}, Hull={self.show_convex_hull}, LEC={self.show_lec}")  # Debug
        self.draw()
    ...
    def on_canvas_click(self, event):
        ...
        # Check bounds
        if (x < self.MARGIN or x > self.CANVAS_WIDTH - self.MARGIN or 
            y < self.MARGIN or y > self.CANVAS_HEIGHT - self.MARGIN):
        ...
        if self.mode == 'sites':
            ...
            if self.selected_obstacle is not None and self.selected_obstacle < len(self.obstacles):
                ...
                self.obstacles_listbox.insert(self.selected_obstacle, 
                ...
                print(f"Created new obstacle: {len(self.obstacles)} obstacles total")  # Debug
                
                # Update obstacles listbox
                self.obstacles_listbox.insert(tk.END, f"Obstacle {len(self.obstacles)} (1 points)")
    ...
    def compute_voronoi(self, sites):
        ...
        edges = []
        voronoi_plot_2d(vor, ax=self.ax, show_vertices=False, 
    ...
    def get_current_obstacle_positions(self, time):
        ...
        for obstacle in self.obstacles:
            if len(obstacle['path']) < 2:
                current_pos = obstacle['path'][0] if obstacle['path'] else {'x': 0, 'y': 0}
    ...
    def compute_lec(self, sites, current_obstacles):
        ...
        step = 15
        for x in range(int(bounds['minX']), int(bounds['maxX']) + 1, step):
            for y in range(int(bounds['minY']), int(bounds['maxY']) + 1, step):
                center = {'x': x, 'y': y}
                
                # Find minimum distance to any site
                min_dist_to_site = float('inf')
                for site in sites:
                    dist = math.sqrt((center['x'] - site['x'])**2 + (center['y'] - site['y'])**2)
                    min_dist_to_site = min(min_dist_to_site, dist)
                ...
                candidate_radius = min(min_dist_to_site, dist_to_bounds)
                
                # Check obstacles
                for obstacle in current_obstacles:
                    dist_to_obstacle = math.sqrt(
                        (center['x'] - obstacle['currentPos']['x'])**2 + 
                        (center['y'] - obstacle['currentPos']['y'])**2
                    )
                    clearance = dist_to_obstacle - obstacle['radius']
                    candidate_radius = min(candidate_radius, clearance)
                    
                if candidate_radius > max_radius and candidate_radius > 0:
                    max_radius = candidate_radius
                    best_circle = {'center': center, 'radius': max_radius}
    ...
    def draw(self):
        ...
        try:
            ...
            # Draw border
            border_x = [self.MARGIN, self.CANVAS_WIDTH - self.MARGIN, 
                       self.CANVAS_WIDTH - self.MARGIN, self.MARGIN, self.MARGIN]
            border_y = [self.MARGIN, self.MARGIN, 
                       self.CANVAS_HEIGHT - self.MARGIN, self.CANVAS_HEIGHT - self.MARGIN, self.MARGIN]
            self.ax.plot(border_x, border_y, color='#dee2e6', linewidth=2)
            
            if len(self.sites) == 0:
                self.ax.text(self.CANVAS_WIDTH // 2, self.CANVAS_HEIGHT // 2, 
                            'Click to add sites', ha='center', va='center', 
            ...
            if self.show_voronoi and len(self.sites) > 1:
                ...
                for edge in voronoi_edges:
                    self.ax.plot([edge['x1'], edge['x2']], [edge['y1'], edge['y2']], 
                               color='#3498db', linestyle='--', linewidth=1, alpha=0.7)
            ...
            if self.show_lec and len(self.sites) >= 2:
                ...
                if lec:
                    ...
                    circle = Circle((lec['center']['x'], lec['center']['y']), lec['radius'],
                                   fill=True, facecolor='#f39c12', alpha=0.1, 
                                   edgecolor='#f39c12', linewidth=2)
                    self.ax.add_patch(circle)
                    # Draw center point
                    self.ax.plot(lec['center']['x'], lec['center']['y'], 'o', 
                               color='#f39c12', markersize=6)
            ...
            for i, obstacle in enumerate(current_obstacles):
                ...
                if len(obstacle['path']) > 1:
                    ...
                    self.ax.plot(path_x, path_y, color='#95a5a6', linestyle='--', 
                               linewidth=2, alpha=0.7)
                               
                # Draw obstacle
                color = '#e67e22' if i == self.selected_obstacle else '#8e44ad'
                circle = Circle((obstacle['currentPos']['x'], obstacle['currentPos']['y']), 
                               obstacle['radius'], fill=True, facecolor=color, alpha=0.8)
                self.ax.add_patch(circle)
                
                # Draw direction indicator
                self.ax.plot(obstacle['currentPos']['x'], obstacle['currentPos']['y'], 
            ...
            if len(self.temp_path) > 0:
                ...
                self.ax.plot(temp_x, temp_y, color='#e74c3c', linestyle=':', 
                            linewidth=2, alpha=0.7)
                            
            self.ax.set_title('Dynamic LEC with Voronoi Diagrams', fontsize=14, fontweight='bold')
