...
class InteractiveVoronoiPlotter:
    ...
    def setup_plot(self):
        ...
        # Styling
        self.ax.set_title(
            "Interactive Voronoi Diagram Plotter\n"
    ...
    def remove_nearest_point(self, x, y):
        ...
        distances = [
            np.linalg.norm(np.array(point) - click_point) for point in all_points
        ]
        ...
        if point_types[nearest_idx] == "site":
            ...
            obstacle_idx = sum(
                1 for i in range(nearest_idx) if point_types[i] == "obstacle"
            )
            removed_point = self.obstacles.pop(obstacle_idx)
            print(
                f"Removed obstacle at ({removed_point[0]:.2f}, {removed_point[1]:.2f})"
            )
    ...
    def generate_random_points(self, n_sites=8, n_obstacles=3):
        ...
        for _ in range(n_sites):
            ...
            while attempts < 50:  # Prevent infinite loops
                ...
                for existing_site in sites:
                    if (
                        np.linalg.norm(np.array(new_site) - np.array(existing_site))
                        < 0.5
                    ):
                        too_close = True
                        break
        ...
        print(
            f"Generated {len(self.sites)} random sites and {len(self.obstacles)} obstacles"
        )
        self.compute_all()
        self.update_plot()
                ...
                if -1 in simplex and len(simplex) == 2:  # Infinite edge
                    try:
                        ...
                        if len(finite_vertices) > 0:
                            ...
                            t = (
                                self.voronoi.points[pointidx[1]]
                                - self.voronoi.points[pointidx[0]]
                            )
                            ...
                            if t_norm > 0:
                                ...
                                if np.all(np.abs(vertex) < 1000):  # Sanity check
                                    ...
                                    # Clip to plot bounds
                                    if (
                                        0 <= vertex[0] <= self.width
                                        and 0 <= vertex[1] <= self.height
                                    ):
                                        self.ax.plot(
                                            [vertex[0], far_point[0]],
            ...
            try:
                for simplex in self.voronoi.ridge_vertices:
                    ...
                    if len(simplex) == 2 and -1 not in simplex:
                        points = self.voronoi.vertices[simplex]
                        if np.all(np.isfinite(points)) and np.all(
                            np.abs(points) < 1000
                        ):
                            self.ax.plot(
                                points[:, 0],
    ...
    def plot_largest_empty_circle(self):
        ...
        circle = Circle(
            ...
            radius,
            fill=True,
            facecolor=self.colors["largest_circle_fill"],
            alpha=0.2,
            edgecolor=self.colors["largest_circle"],
            linewidth=2,
            linestyle=":",
            label=f"Largest Empty Circle (r={radius:.2f})",
        )
        ...
        self.ax.plot(
            ...
            center[1],
            "o",
            color=self.colors["largest_circle"],
            markersize=8,
    ...
    def on_key_press(self, event):
        ...
        if key == "c":
            ...
            print(
                f"Largest empty circle: {'ON' if self.show_largest_circle else 'OFF'}"
            )
            self.update_plot()
        elif key == "s":
            self.save_plot()
