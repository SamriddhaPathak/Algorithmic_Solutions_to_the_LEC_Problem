...
class VoronoiLECPlotter:
    def __init__(self, root):
        ...
        self.status_var.set(
            "Ready. Click to add sectors. Use toolbar to zoom/pan. Home button to reset view."
        )
    ...
    def create_ui(self):
        ...
        title_label = ttk.Label(
            control_frame, text="Voronoi & LEC Solver", font=("Helvetica", 14, "bold")
        )
        ...
        instruction_label = ttk.Label(
            control_frame,
            text="Click on the right panel to add sectors or use manual entry below.\nThen click 'Generate' to create the diagram and find the largest empty circle.",
            wraplength=280,
        )
        ...
        add_manual_button = ttk.Button(
            manual_frame, text="Add Sector", command=self.add_manual_point
        )
        ...
        self.convex_hull_checkbox = ttk.Checkbutton(
            options_frame,
        ...
        self.generate_button = ttk.Button(
            buttons_frame,
            text="Generate Voronoi & LEC",
            command=self.generate_voronoi_and_lec,
        )
        ...
        self.vertices_text = scrolledtext.ScrolledText(
            vertices_frame, height=6, wrap=tk.WORD
        )
    ...
    def redraw_plot(self):
        ...
        if self.voronoi_result is not None:
            ...
            if (
                self.largest_empty_circle is not None
                and self.largest_empty_circle["center"] is not None
            ):
    ...
    def generate_voronoi_and_lec(self):
        ...
        self.status_var.set(
            "Voronoi diagram and Largest Empty Circle generated successfully!"
        )
    ...
    def find_largest_empty_circle(self, vor):
        ...
        # Store the result
        self.largest_empty_circle = {"center": best_center, "radius": best_radius}
    ...
    def update_information_panels(self):
        ...
        for i, point in enumerate(self.points):
            self.sites_text.insert(
                tk.END, f"Site {i+1}: ({point[0]:.4f}, {point[1]:.4f})\n"
            )
        ...
        if self.voronoi_result is not None:
            ...
            for i, vertex in enumerate(self.voronoi_result.vertices):
                self.vertices_text.insert(
                    tk.END, f"Vertex {i+1}: ({vertex[0]:.4f}, {vertex[1]:.4f})\n"
                )
        ...
        if self.largest_empty_circle is not None:
            ...
            if center is not None:
                self.lec_text.insert(
                    tk.END, f"Center: ({center[0]:.4f}, {center[1]:.4f})\n"
                )
                ...
                for idx in nearest_indices:
                    ...
                    if abs(distance - radius) < 0.001:  # Consider it's on boundary
                        self.lec_text.insert(
                            tk.END,
                            f"Site {idx+1}: ({points[idx][0]:.4f}, {points[idx][1]:.4f}) - On boundary\n",
                        )
                    else:
                        self.lec_text.insert(
                            tk.END,
                            f"Site {idx+1}: ({points[idx][0]:.4f}, {points[idx][1]:.4f}) - Distance: {distance:.4f}\n",
                        )
            else:
        ...
        if self.convex_hull is not None:
            ...
            self.hull_text.insert(
                tk.END, f"Number of vertices: {len(self.convex_hull.vertices)}\n"
            )
            ...
            for i, vertex_idx in enumerate(self.convex_hull.vertices):
                ...
                self.hull_text.insert(
                    tk.END,
                    f"Vertex {i+1}: Site {vertex_idx+1} at ({point[0]:.4f}, {point[1]:.4f})\n",
                )
        else:
            self.hull_text.delete(1.0, tk.END)
            if len(self.points) < 3:
                self.hull_text.insert(
                    tk.END, "Need at least 3 points to compute convex hull.\n"
                )
            else:
